(function () {
    ...
    const exec = {
        ...
        INC: (pdp8_obj) => {
             ...
             */
            if (pdp8_obj.status.clock === 3) {
                let tmp = (pdp8_obj.registers.E << 16) + pdp8_obj.registers.AC + 1;
                pdp8_obj.registers.E = (tmp & 0b10000000000000000) >>> 16;
        ...
        ADD: (pdp8_obj) => {
            ...
            // E-AC <- AC + MBR
                ...
                let tmp = (pdp8_obj.registers.AC + pdp8_obj.registers.MBR) & 0b1111111111111111;
                if (tmp > 32767) {
                ...
                if ((((pdp8_obj.registers.AC & mask) >> 15) === 1 && ((pdp8_obj.registers.MBR & mask) >> 15) === 1 && ((tmp & mask) >> 15) === 0) ||
                    (((pdp8_obj.registers.AC & mask) >> 15) === 0 && ((pdp8_obj.registers.MBR & mask) >> 15) === 0 && ((tmp & mask) >> 15) === 1)) {
    ...
    class PDP8 {
        ...
        compile(text) {
            ...
            if (Object.keys(to_resolve).length !== 0) {
                for (let mem_add in to_resolve) {
                    errors[`Error (LABEL[${to_resolve[mem_add]}])`] = `Missing LABEL reference!`;
                }
            } 
