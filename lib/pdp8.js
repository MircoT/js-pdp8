(function () {
    ...
    const fetch = (pdp8_obj) => {
        ...
        if (pdp8_obj.clock === 0) {
            ...
            pdp8_obj.currnet_add_cmd = pdp8_obj.registers.MAR;
        }
        // MBR <- M , PC <- PC+1
        else if (pdp8_obj.clock === 1) {
            pdp8_obj.registers.MBR = pdp8_obj.ram.get(pdp8_obj.registers.MAR);
    ...
    class PDP8 {
        ...
        step() {
            if (this.ctrlUnit.S) {                
                //console.log("STEP", this.ctrlUnit.F, this.ctrlUnit.R, this.clock)
        ...
        next() {
            let cur_add = this.currnet_add_cmd;
            
            if (cur_add === -1) {
                this.step();
                cur_add = this.currnet_add_cmd;
            }
            
            while (this.currnet_add_cmd === cur_add && this.ctrlUnit.S) {
                this.step();
        ...
        reset() {
            ...
            this.registers.PC = this.start_add;
            this.currnet_add_cmd = -1;
            this.clock = 0;
            return this;
        }
        ...
        getCodeRef() {
            ...
            for (var key of this.code_ref.keys()) {
                if (this.code_ref.get(key) === this.registers.PC) {
                    ...
                    references.PC.ram = this.code_ref.get(key) - this.start_add + 1;
                }
                if (this.code_ref.get(key) === this.registers.MAR) {
                    references.MAR.source = key;
                    references.MAR.ram = this.code_ref.get(key) - this.start_add + 1;
                }
            }
                ...
                // ----- ORG -----
                    ...
                    if ((cur_tok.type === types.HEX_NUM ||
                        ...
                        parseInt(cur_tok.val, 16) >= 0) {
                            this.start_add = parseInt(cur_tok.val, 16);
