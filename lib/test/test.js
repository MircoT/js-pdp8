...
describe('js-pdp8', function() {
  ...
  describe('I/O Istructions', function () {
    ...
    it("ION", function () {
        ...
        vm.reset()
        vm.getCtrlUnit().INT = false;
        vm.start()
          .next();
        assert.strictEqual(vm.getCtrlUnit().INT, true, 'ION have to set INT to true in CtrlUnit');
    });
    it("IOF", function () {
        var vm = new PDP8();
        vm.compile("IOF");
        ...
        vm.reset()
          .start()
          .next();
        assert.strictEqual(vm.getCtrlUnit().INT, false, 'IOF have to set INT to false in CtrlUnit');
    });
  });
        ...
        if (cur_file.slice(0, 4) !== 'res_') {
            it('Compile ' + cur_file, function () {
                ...
                assert.strictEqual(vm.getRam(), 
                    fs.readFileSync(path.join(__dirname, 'sources', 'res_'+cur_file), { encoding: "utf-8" }), 
  ...
  describe('Test execution', function () {
    it('Overflow check', function () {
        ...
        assert.strictEqual(vm.getCtrlUnit().S, false, "VM have to stop at the end of overflow test");
    });
    ...
    it('Code Ref. and Stop', function () {
        ...
        assert.strictEqual(vm.getCtrlUnit().S, true, 'VM have to start');
        
        vm.stop();
        
        assert.strictEqual(vm.getCtrlUnit().S, false, 'VM have to stop');
        ...
        expect(vm.getCodeRef()).to.deep.equal({ PC: { source: 7, ram: 5 }, MAR: { source: 6, ram: 4 } });
    });
